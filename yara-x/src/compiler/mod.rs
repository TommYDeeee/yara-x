...
impl<'a> Compiler<'a> {
    ...
    pub fn add_source(&mut self, src: &str) -> Result<&mut Self, Error> {
        ...
        for rule in ast2.tree().rules() {
            println!("rule: {:?}", rule.identifier_token().unwrap().text());
            println!(
                "rule: {:?}",
                rule.identifier_token().unwrap().text_range()
            );
        }

        println!("yara-x parser: {:.2?}", elapsed);
        println!("new parser: {:.2?}", elapsed2);
...
impl<'a> Compiler<'a> {
    ...
    fn check_for_existing_identifier(
        &self,
        ident: yara_parser::SyntaxToken,
    ) -> Result<(), CompileError> {
        println!("ident name: {:?}", ident.text());
        println!("ident span: {:?}", ident.text_range());
        if let Some(symbol) = self.symbol_table.lookup(ident.text()) {
...
impl<'a> Compiler<'a> {
    fn c_rule(&mut self, rule: Rule) -> Result<(), CompileError> {
        // Check if another rule, module or variable has the same identifier
        // and return an error in that case.
        self.check_for_existing_identifier(rule.identifier_token().unwrap())?;
        ...
        self.rules.push(RuleInfo {
            ...
            ident_id: self
                .ident_pool
                .get_or_intern(rule.identifier_token().unwrap().text()),
            ident_span: Span::new(
                SourceId(0),
                rule.identifier_token().unwrap().text_range().start().into(),
