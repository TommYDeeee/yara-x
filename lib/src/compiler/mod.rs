...
impl<'a> Compiler<'a> {
        ...
        S: Into<SourceCode<'src>>,
    {
        ...
        if let Err(err) = utf8_validation {
            ...
            return Err(Error::from(Box::new(CompileError::invalid_utf_8(
                &self.report_builder,
        ...
        if !ast2.errors().is_empty() {
            for error in ast2.errors() {
                let error = Error::from(Box::new(CompileError::syntax_error(
                    &self.report_builder,
                    error.to_string(),
                    Span::new(
                        SourceId(0),
...
impl<'a> Compiler<'a> {
    fn c_rule(&mut self, rule: Rule) -> Result<(), Box<CompileError>> {
